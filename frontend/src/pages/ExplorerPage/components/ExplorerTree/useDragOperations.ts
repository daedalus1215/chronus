...
function parseItemId(dndId: string): { type: 'folder' | 'note'; id: number } {
  if (dndId.startsWith('folder-'))
    return { type: 'folder', id: parseInt(dndId.slice(7), 10) };
...
export const useDragOperations = ({
  ...
  const updateIntent = (intent: DropIntent) => {
    if (
      intent?.type === dropIntentRef.current?.type &&
      intent?.overId === dropIntentRef.current?.overId
    )
      return;
  ...
  const onDragMove = useCallback((event: DragMoveEvent) => {
    ...
    const inMiddle =
      Math.abs(activeCenterY - overCenterY) <= over.rect.height * 0.25;
  ...
  const onDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      if (activeItem.type === 'folder' && overItem.type === 'folder') {
        ...
        try {
          await reorderFolders({ items, parentId });
        } catch {
          setFolders(snapshot);
        }
        return;
      ...
      if (activeItem.type === 'note' && overItem.type === 'note') {
        ...
        try {
          await reorderNotes({ items, folderId });
        } catch {
          setNotes(snapshot);
        }
      }
