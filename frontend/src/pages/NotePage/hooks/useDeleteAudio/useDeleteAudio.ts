...
import { deleteAudio, NoteAudio } from '../../../../api/requests/audio.requests';
...
const createNoteAudiosQueryKey = (noteId: number): readonly [string, number] => {
...
export const useDeleteAudio = (noteId: number) => {
  ...
  return useMutation<void, Error, number, DeleteAudioMutationContext>({
    ...
    onMutate: async (audioId: number) => {
      ...
      const previousAudiosResponse = queryClient.getQueryData<NoteAudiosResponse>(
        noteAudiosQueryKey
      );
      queryClient.setQueryData<NoteAudiosResponse | undefined>(
        ...
        oldAudiosResponse => {
          ...
          return {
            ...oldAudiosResponse,
            audios: oldAudiosResponse.audios.filter(audio => audio.id !== audioId),
          };
        }
      );
      return { previousAudiosResponse };
    },
    onError: (_error: Error, _audioId: number, context?: DeleteAudioMutationContext) => {
      if (context?.previousAudiosResponse) {
        queryClient.setQueryData(noteAudiosQueryKey, context.previousAudiosResponse);
      }
    },
    onSettled: async () => {
